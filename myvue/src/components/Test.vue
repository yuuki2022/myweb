<template>
          ...
          <el-row gutter="20">
            <div class="buttonContainer buttons">
              <el-scrollbar height="400px">
                ...
                :class="{ 'selected': question.choice !== null }"
                >{{index+1}}</el-button>
              </el-scrollbar>                    
            </div>                        
          </el-row>
          ...
          <el-row gutter="20">
            <div>
              <el-scrollbar height="650px">
                ...
                <el-card v-for="(question,index) in questions" :key="question.questionId" class="questionCard" :id="'question-' + question.questionId">
        ...
        <el-col :span="3"  :offset="1" style="text-align: center;">
          <el-row gutter="20">
            <!--el-card shadow="hover" style="width: 100%;margin-top: 20px; display: flex; flex-direction: column; align-items: center; height:80%"-->
              <div style="width: 100%; background: #F8F9FB; height:140px ;display: flex;flex-direction: column; align-items: center;" >
                <h3 style="color: red; margin-bottom: 0px;" v-show="visible">考试剩余时间</h3>
                <el-countdown v-show="visible" format="HH:mm:ss" :value="value1" value-style="color:red ; font-size:40px ;font-weight:bold"/>
                <el-button class="countdown-footer" type="primary" @click="visible=!visible" >{{ visible ? '隐藏' : '显示' }}考试剩余时间</el-button>
              </div>
            <!--/el-card-->
          </el-row>
          <el-row>
  ...
  export default {
    ...
    methods:{
      ...
      if (questionElement) {
        ...
        fetchQuestions() {
      axios.get('D:\作业\软件开发实训\项目文件\test-demo\文档\jsons\questions.json')
        .then(response => {
          // 解析JSON数据
  ...
  .el-countdown {
    visibility: hidden; /* 使用 visibility 隐藏元素 */
  }
  .scrollbar-demo-item {
    display: flex;
    align-items: center;
  ...
  .button-container {
    ...
    gap: 40px; /* 设置按钮之间的垂直间距 */
  }
  
  .button-container > * {
    flex: 1; /* 让每个按钮在垂直方向上等分剩余空间 */
  }
